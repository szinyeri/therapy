...
function Init()
  ...
  --  print ("env init: "..environment.init()) -- inicializálja a hőmérőt a default potokon scl=5 sda=6
    local msg = "startup.lua - Init() -"
    local res = environment.init(i2c_sda, i2c_scl)
    ...
    sntp.sync("pool.ntp.org",
     ...
     tmr.alarm(3, 2*1000, tmr.ALARM_SINGLE, function() 
        get_env()
        tmr.alarm(3, 3*1000, tmr.ALARM_AUTO,get_env)
        end)

-- figyeli, hogy mikor kell kikapcsolni a készüléket
    tmr.alarm(1, 5*60*1000, tmr.ALARM_AUTO, function() 
        if terapia_fut == false then -- kikapcsolja a készüléket
            gpio.write(power_off_pin, gpio.LOW)
        end
       end)
...
function get_env()
    if package.loaded["environment"] == nil then
        require("environment")
        environment.init(i2c_sda, i2c_scl)
        tmr.delay(100)
    end
    lastenv, lasttemp = environment.read()
end
...
WTFunc = Init -- OntInit után GetGWTime jön
WiFiOKWt() -- WiFi check
