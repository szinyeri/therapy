...
function tempmeasure()
  ...
  temps = dofile("gettemps.lua")
 -- print(string.format("temps: %2.1f, %2.1f", temps[1], temps[2]))
 -- print (string.format("environment.lua - tempmeasure() - mert_T=%d.%01d;  humidity=%d.%01d%% hot=%2.1f cold=%2.1f",
       --    env[2]/10, env[2]%10, env[1]/10, env[1]%10), temps[1], temps[2])
        --syslog(string.format("environment.lua - tempmeasure() - mert_T=%d.%01d;  humidity=%d.%01d%% hot=%2.1f cold=%2.1f",
         --  env[2]/10, env[2]%10, env[1]/10, env[1]%10), temps[1], temps[2])
end
